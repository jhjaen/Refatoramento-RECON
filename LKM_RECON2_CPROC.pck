CREATE OR REPLACE PACKAGE LKM_RECON2_CPROC IS
...
*/
/
CREATE OR REPLACE PACKAGE BODY LKM_RECON2_CPROC IS
  MPROC_ID INTEGER;
  --
  ...
  BEGIN
    ...
    FOR VERSAO IN (select VERSAO
                    ...
                    where OBJETO_NOME = 'LKM_RECON2_CPROC'
    ...
    LIB_PROC.add_param(pstr,
                       ...
                       'from dual union all                                      ' ||
                       'SELECT DISTINCT est.cod_estab, est.cod_estab||'' - ''|| substr(nome_fantasia,1,30) || '' - ''|| cidade ' ||
  ...
  BEGIN
    RETURN 'RELATORIO DE APOIO RECONCILIACAO - R E C O N (2.0) - 2020';
  END;
  ...
  BEGIN
    ...
    MPROC_ID := LIB_PROC.NEW('LKM_RECON2_CPROC', 48, 150);
    ...
    -->Aquisição dos grupos do usuário:
    ...
    IF (P_FINALIDADE = '1') THEN
        IF (GRUPO15 = MUSUARIO) THEN
            ...
            LIB_PROC.ADD_LOG('Usuário com permissão no grupo 15 - Recon_Geral_ADM - DARF sendo gerada!', 1);
        ELSE
            LIB_PROC.ADD_LOG('Usuário SEM permissão no grupo 15 - Recon_Geral_ADM - DARF NÃO gerada!', 1);
        END if;
    END if;

 /*   FOR GPR_BLOCK_DARF IN (select DISTINCT USR.USR_LOGIN
                             from usuario_estab UEST
                             left join pl_usr USR
                               on USR.USR_LOGIN = UEST.COD_USUARIO
                             left join pl_grp_usr GUSR
                               on GUSR.USR_KEY = USR.USR_KEY
                            WHERE uest.cod_usuario = MUSUARIO
                              and GUSR.GRP_KEY = '15') LOOP

      IF (P_FINALIDADE = '1') AND GPR_BLOCK_DARF.USR_LOGIN = MUSUARIO THEN
      ...
      LIB_PROC.ADD_LOG('Gerado com Sucesso: DARF ', 1);

    END LOOP;

    FOR GPR_S_PREMS_DARF IN (WITH LKM_BLOC as
                                (select DISTINCT USR.USR_LOGIN,
                                                GUSR.GRP_KEY GRUPO
                                  from usuario_estab UEST
                                  left join pl_usr USR
                                  ...
                                  left join pl_grp_usr GUSR
                                    on GUSR.USR_KEY = USR.USR_KEY
                                 WHERE uest.cod_usuario = MUSUARIO
                                   and GUSR.GRP_KEY in ('10', '14', '15'))
                               select DISTINCT USR_LOGIN, SUM(GRUPO) GRUPO
                                 from LKM_BLOC
                                group by USR_LOGIN) LOOP

      IF (P_FINALIDADE = '1') AND GPR_S_PREMS_DARF.USR_LOGIN = MUSUARIO AND
         GPR_S_PREMS_DARF.GRUPO in ('14', '24') THEN

        LIB_PROC.ADD_LOG(' ', 1);
        LIB_PROC.ADD_LOG('      >>>>>>>  USUARIO :' || MUSUARIO ||
                         ' NAO ESTA HABILITADO PARA ESSA GERACAO, ESTE USARIO ESTA LIBERADO SOMENTE PARA GERACOES DA DAM - CASO NECESSARIO ENTRAR EM CONTATO COM ADMINISTRADOR  <<<<<<<',
                         1);
        LIB_PROC.ADD_LOG(' ', 1);
      ...
      IF (P_FINALIDADE = '1') AND GPR_S_PREMS_DARF.USR_LOGIN = MUSUARIO AND
         GPR_S_PREMS_DARF.GRUPO = '39' THEN

        LIB_PROC.ADD_LOG(' ', 1);
        LIB_PROC.ADD_LOG('      >>>>>>>  USUARIO :' || MUSUARIO ||
                         ' NAO ESTA HABILITADO PARA ESSA GERACAO,',
                         1);
        LIB_PROC.ADD_LOG(' ESTE USARIO ESTA HABILITADO NO GRUPO FILIAIS, CASO ESSE USARIO SEJA DA SEDE SOLICITAR A EXCLUSAO DO GRUPO FILIAIS',
                         1);
        LIB_PROC.ADD_LOG(' CASO NECESSARIO ENTRAR EM CONTATO COM ADMINISTRADOR  <<<<<<<',
                         1);
        LIB_PROC.ADD_LOG(' ', 1);
      ...
      IF (P_FINALIDADE = '1') AND GPR_S_PREMS_DARF.USR_LOGIN = MUSUARIO AND
         GPR_S_PREMS_DARF.GRUPO is null THEN

        LIB_PROC.ADD_LOG(' ', 1);
        LIB_PROC.ADD_LOG('      >>>>>>>  USUARIO :' || MUSUARIO ||
                         ' NAO ESTA HABILITADO PARA ESSA GERACAO, ESTE USARIO NAO FOI HABILITADO EM NENHUM GRUPO - CASO NECESSARIO ENTRAR EM CONTATO COM ADMINISTRADOR  <<<<<<<',
                         1);
        LIB_PROC.ADD_LOG(' ', 1);
      ...
      IF (P_FINALIDADE = '1') AND GPR_S_PREMS_DARF.USR_LOGIN = MUSUARIO AND
         GPR_S_PREMS_DARF.GRUPO in ('10') THEN

        LIB_PROC.ADD_LOG(' ', 1);
        LIB_PROC.ADD_LOG('      >>>>>>>  USUARIO :' || MUSUARIO ||
                         ' NAO ESTA HABILITADO PARA ESSA GERACAO, ESTE USARIO NAO FOI HABILITADO NO GRUPO FILIAIS - NECESSARIO HABILITAR O GRUPO RECON_FILIAS - CASO NECESSARIO ENTRAR EM CONTATO COM ADMINISTRADOR  <<<<<<<',
                         1);
        LIB_PROC.ADD_LOG(' ', 1);
    ...
    ---------------------------- DAM
     IF (P_FINALIDADE = '2') or (P_FINALIDADE = 5) THEN
        IF (GRUPO1 = MUSUARIO) or (GRUPO14 = MUSUARIO) THEN
            ...
            LIB_PROC.ADD_LOG('Usuário com permissão no grupo 1 ou 14 - ADM ou Recon_DAM_Filiais - DAM sendo gerada!', 1);
        ELSE
            LIB_PROC.ADD_LOG('Usuário SEM permissão no grupo 1 ou 14 - ADM ou Recon_DAM_Filiais - DAM NÃO gerada!', 1);
        END if;
    END if;
   /* FOR GPR_BLOCK IN (select DISTINCT USR.USR_LOGIN
                        from usuario_estab UEST
                        left join pl_usr USR
                          on USR.USR_LOGIN = UEST.COD_USUARIO
                        left join pl_grp_usr GUSR
                          on GUSR.USR_KEY = USR.USR_KEY
                       WHERE uest.cod_usuario = MUSUARIO
                         and GUSR.GRP_KEY in ('14', '1')) LOOP

      IF (P_FINALIDADE = '2') OR (P_FINALIDADE = '5' ) AND GPR_BLOCK.USR_LOGIN = MUSUARIO THEN
        ...
        LIB_PROC.ADD_LOG('Gerado com Sucesso: DAM ', 1);
    ...
    FOR GPR_S_PREMS_DARF IN (WITH LKM_BLOC as
      ...
      v_contadorLOG := v_contadorLOG + 1;

      IF (P_FINALIDADE = '1') AND GPR_S_PREMS_DARF.USR_LOGIN = MUSUARIO AND
         GPR_S_PREMS_DARF.GRUPO in ('39') THEN

        LIB_PROC.ADD_LOG(' ', 1);
        LIB_PROC.ADD_LOG('      >>>>>>>  USUARIO :' || MUSUARIO ||
                         ' NAO ESTA HABILITADO PARA ESSA GERACAO, ESTE USARIO ESTA LIBERADO NO GRUPO FILIAIS  - CASO ESSE USUARIO SEJA DA SEDE NECESSARIO ENTRAR EM CONTATO COM ADMINISTRADOR PARA EXCLUIR O GRUPO FILIAIS <<<<<<<',
                         1);
        LIB_PROC.ADD_LOG(' ', 1);

      END IF;

      IF (P_FINALIDADE = '2') OR (P_FINALIDADE = '5' ) AND GPR_S_PREMS_DARF.USR_LOGIN = MUSUARIO AND
         GPR_S_PREMS_DARF.GRUPO = '15' THEN

        LIB_PROC.ADD_LOG(' ', 1);
        LIB_PROC.ADD_LOG('      >>>>>>>  USUARIO :' || MUSUARIO ||
                         ' NAO ESTA HABILITADO PARA ESSA GERACAO,',
                         1);
        LIB_PROC.ADD_LOG(' ESTE USARIO ESTA HABILITADO NO GRUPO RECON_ADM E NAO ESTA LIBERADO NO GRUPO RECON FILIAIS , CASO ESSE USARIO SEJA DA SEDE SOLICITAR A INCLUSAO DO GRUPO RECON_FILIAIS',
                         1);
        LIB_PROC.ADD_LOG(' CASO NECESSARIO ENTRAR EM CONTATO COM ADMINISTRADOR  <<<<<<<',
                         1);
        LIB_PROC.ADD_LOG(' ', 1);
    ...
    IF (P_FINALIDADE = '3') THEN
        ...
        END if;
    END if;
   /* FOR GPR_BLOCK_GPS IN (select DISTINCT USR.USR_LOGIN
                            from usuario_estab UEST
                            left join pl_usr USR
                              on USR.USR_LOGIN = UEST.COD_USUARIO
                            left join pl_grp_usr GUSR
      ...
      IF (P_FINALIDADE = '3') AND GPR_BLOCK_GPS.USR_LOGIN = MUSUARIO THEN
        ...
        LIB_PROC.ADD_LOG('Gerado com Sucesso: GPS ', 1);
    ...
    FOR GPR_S_PREMS_DARF IN (WITH LKM_BLOC as
      ...
      IF (P_FINALIDADE = '3') AND GPR_S_PREMS_DARF.USR_LOGIN = MUSUARIO AND
         GPR_S_PREMS_DARF.GRUPO in ('14', '24') THEN
    ...
    IF (P_FINALIDADE = '4') THEN
        IF (GRUPO15 = MUSUARIO) THEN
            ...
            LIB_PROC.ADD_LOG('Usuário com permissão no grupo 15 - Recon_Geral_ADM - REINF sendo gerada!', 1);
        ELSE
            LIB_PROC.ADD_LOG('Usuário SEM permissão no grupo 15 - Recon_Geral_ADM - REINF NÃO gerada!', 1);
        END if;
    END if;
    /*FOR GPR_BLOCK_GPS IN (select DISTINCT USR.USR_LOGIN
                            from usuario_estab UEST
      ...
      IF (P_FINALIDADE = '4') AND GPR_BLOCK_GPS.USR_LOGIN = MUSUARIO THEN
        ...
        LIB_PROC.ADD_LOG('Gerado com Sucesso: RECON ', 1);
    ...
    FOR GPR_S_PREMS_DARF IN (WITH LKM_BLOC as
      ...
      IF (P_FINALIDADE = '4') AND GPR_S_PREMS_DARF.USR_LOGIN = MUSUARIO AND
         GPR_S_PREMS_DARF.GRUPO = '39' THEN
  ...
  BEGIN
    ...
    FOR MINST1 IN (SELECT DISTINCT BKTXT, xblnr
      ...
      IF MINST1.BKTXT = 'DARF' THEN
        ...
        -- V_data_ini_A := to_date(PS_DATA_INICIAL ,'DD/MM/YYYY');
        -- V_data_fim_A := to_date(PS_DATA_FINAL ,'DD/MM/YYYY');
...
--                                            '                   '''' FORNEC                                  ' ||
  ...
  END IF;
    ...
    FOR MINST2 IN (SELECT DISTINCT BKTXT
      ...
      IF MINST2.BKTXT = 'DAM' THEN
        ...
        --ENTIDADES DAM
        /*Script_DAM_executar_Entid := 'INSERT INTO LKM_SAIDA_GPS_DARF_ITEM1  SELECT                          ' ||
                                       ' ''I'' ITEM,                                                          ' ||
                                       ' LIFNR FORNECEDOR,                                                    ' ||
                                       ' COD_ESTAB LOCALNEGOCIO,                                             ' ||
                                       ' TRIM(TO_CHAR(SUM(ENTIDADES),''000000000099'')) VLRTRIB,                ' ||
                                       ' ''I;ENTIDADES;'' ||  COD_ESTAB || '';'' || TRIM(TO_CHAR(SUM(ENTIDADES),''000000000099''))|| '';'' AS LINHA,' ||
                                       ' LIFNR COD_FIS_JUR,                                                   ' ||
                                       ' ZUONR NUM_DOC_FIS,                                                   ' --ZUONR
                                       ||
                                       ' BURKS EMPRESA,                                                       ' ||
  ...
  BEGIN
    ...
    END IF;
        ...
        --ENTIDADES GPS
        /*Script_GPS_executar_Entid := 'INSERT INTO LKM_SAIDA_GPS_DARF_ITEM1  SELECT                          ' ||
                                       ' ''I'' ITEM,                                                          ' ||
                                       ' LIFNR FORNECEDOR,                                                    ' ||
                                       ' substr(LOCAL_NEGO,1,4) LOCALNEGOCIO,                                 ' ||
                                       ' TRIM(TO_CHAR(SUM(ENTIDADES),''000000000099'')) VLRTRIB,                ' ||
                                       ' ''I;ENTIDADES;'' || substr(LOCAL_NEGO,1,4) || '';'' || TRIM(TO_CHAR(SUM(ENTIDADES),''000000000099'')) || '';'' AS LINHA,' ||
                                       ' LIFNR COD_FIS_JUR,                                                   ' ||
                                       ' ''0'' NUM_DOC_FIS,                                                   ' || --ZUONR
                                       ' BURKS EMPRESA,                                                       ' ||
                                       ' to_char(LAST_DAY(BLDAT), ''DDMMYYYY'') DATADOC,                                                       ' ||
...
--- REINF 2021
 ...
 PROCEDURE LKM_INSERT_REINF(PS_EMPRESA      VARCHAR2,
  ...
  BEGIN
  /*  FOR MINST1 IN (SELECT DISTINCT count(1) BKTXT
                     FROM VW_LKM_REINF
                    WHERE COD_EMPRESA = PS_EMPRESA
                      AND BLDAT BETWEEN PS_DATA_INICIAL AND PS_DATA_FINAL) LOOP
      IF MINST1.BKTXT > '0' THEN*/
    ...
    END IF;
       ...
       --    dbms_output.put_line(Script_REINF_executar_Multa);
        ...
        --ENTIDADES REINF
        /*Script_REINF_executar_Entid := 'INSERT INTO LKM_SAIDA_REINF_DARF_ITEM1  SELECT                          ' ||
                                       ' ''I'' ITEM,                                                          ' ||
                                       ' LIFNR FORNECEDOR,                                                    ' ||
                                       ' substr(LOCAL_NEGO,1,4) LOCALNEGOCIO,                                 ' ||
                                       ' TRIM(TO_CHAR(SUM(ENTIDADES),''000000000099'')) VLRTRIB,                ' ||
 ...
 ------
  ...
  PROCEDURE LKM_PLOAD(PS_EMPRESA      VARCHAR2,
    ...
    ---Variavel para quebra
    /* qtd_nota            int := 1000;
    controle_qtd        int := 0;
    controle_arq        int := 0;
    num_arq             int := 0;
    num_arq_rodape      int := 0;
    qtd_total_notas     int;
    qtd_mod             int;*/
    ...
    IF (PS_EXPORT = '0') THEN
      ...
      /*    ELSIF C_NFS%NOTFOUND THEN
          ...
          FOR MREG IN C_NFS LOOP
            ...
            VS_MLINHA := NULL;
            VS_MLINHA := LIB_STR.W(VS_MLINHA,
                                   '                            *    S E M    M O V I M E N T O    C/     R E T E N C A O    P A R A   P A G A M E N T O        *',
                                   1);
            LIB_PROC.ADD(VS_MLINHA);
...
/*
    BEGIN
      ...
      FOR MREG2 IN (select 'COD_EMPRESA' || ';' || 'COD_ESTAB' || ';' ||
                           'ALQ' || ';' || 'NUM_CONTROLE' || ';' || 'CHAVE' || ';' ||
                           'COD_ATRIB' || ';' || 'NUM_DOCTO' || ';' ||
                           'SERIE' || ';' || 'TIP_DOCTO' || ';' ||
                           'DATA_DOCTO' || ';' || 'CHAV_LANCTO' || ';' ||
                           'MONTANTE' || ';' || 'TEXTO' AS LINHA,
                           0 CHAVE_ORDERN

                      from DUAL
                    UNION ALL
                    select OK.COD_EMPRESA || ';' || OK.COD_ESTAB || ';' ||
                           OK.ALQ || ';' || OK.NUM_CONTROLE || ';' ||
                           OK.CHAVE || ';' || OK.COD_ATRIB || ';' ||
                           OK.NUM_DOCTO || ';' || OK.SERIE || ';' ||
                           OK.TIP_DOCTO || ';' || OK.DATA_DOCTO || ';' ||
                           OK.CHAV_LANCTO || ';' || OK.MONTANTE || ';' ||
                           OK.TEXTO AS LINHA,
                           ROWNUM CHAVE_ORDERN

                      from LKM_FBL3N_CONC_INSS OK
                     where 1 = 1
                       and COD_EMPRESA = PS_EMPRESA
                       and COD_ESTAB = PS_ESTAB
                       and data_DOCTO between PS_DATA_INICIAL and
                           PS_DATA_FINAL
                       AND OK.Cod_Estab =
                           decode(PS_ESTAB, '000000', OK.cod_estab, PS_ESTAB)
                     ORDER BY CHAVE_ORDERN)

       LOOP

        MLINHA2 := NULL;
        MLINHA2 := LIB_STR.W(MLINHA2, MREG2.LINHA, 1);
        --v_numero_arq := 1;
    ...
    BEGIN
      ...
      FOR MREG3 IN (select 'COD_EMPRESA' || ';' || 'COD_ESTAB' || ';' ||
                           'ALQ' || ';' || 'NUM_CONTROLE' || ';' || 'CHAVE' || ';' ||
                           'COD_ATRIB' || ';' || 'NUM_DOCTO' || ';' ||
                           'SERIE' || ';' || 'TIP_DOCTO' || ';' ||
                           'DATA_DOCTO' || ';' || 'CHAV_LANCTO' || ';' ||
       ...
       LOOP
        ...
        --MLINHA2 := NULL;
        MLINHA2 := LIB_STR.W(MLINHA3, MREG3.LINHA, 1);
        --v_numero_arq := 1;
        ...
        LIB_PROC.add(plinha          => mLinha3,
                     ...
                     plin            => 1,
                     ptipo           => to_number(3),
                     pChaveOrdenacao => MREG3.CHAVE_ORDERN);
